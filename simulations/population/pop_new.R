...
raw_csv_results.48 = read.csv('population/48.csv')[,0:30]
raw_csv_results.64 = read.csv('population/64.csv')[,0:30]
raw_csv_results.96 = read.csv('population/96.csv')[,0:30]

parse_csv_data <- function(raw_csv_results) {
...
results.pop32 <- parse_csv_data(raw_csv_results.32)
results.pop48 <- parse_csv_data(raw_csv_results.48)
results.pop64 <- parse_csv_data(raw_csv_results.64)
results.pop96 <- parse_csv_data(raw_csv_results.96)
...
plot.pop_comp <- ggplot(melted, aes(x=setting, y=value, color=population, group=population)) + 
  stat_summary(fun.max = function(y) max(y), fun.min = function(y) min(y), geom = "errorbar") +
  stat_summary(fun.data = "mean_se", geom = "line", size = 1)  +
  facet_wrap(~population, ncol = 4) +
  theme(strip.text = element_blank()) +
  labs(x = "Settings", y = "Cost", color = "Population")
